...
contract.skip("05 SnailToken - Two snails breedMateAFertilised", async accounts => {
    ...
    describe("Breed Snails: Both mates must be present", () => {
        ...
        it("should NOT allow breedMateAFertilised if neither mate is present (owned/approved)", async () => {

            await truffleAssert.reverts(
                snailToken.breedMateAFertilised(
        ...
        it("should NOT allow breedMateAFertilised if only one mate is present (owned/approved)", async () => {
        ...
        it("should NOT allow a snail to be breedMateAFertilised with itself", async () => {
    ...
    describe("Breed Two Snails (with pseudo-random chance that each mate produces 1x newborn smail): ", () => {
        ...
        it("should allow suitable mates to breedMateAFertilised", async () => {
        ...
        it("should get expected mates A fertilised from each breedMateAFertilised (test 3x)", async () => {

            // Breed (for 1st-time)
            let txBreedResult1
            await truffleAssert.passes(
                txBreedResult1 = await snailToken.breedMateAFertilised(
                    A_SNAIL_ID, //mateA                
                    B_SNAIL_ID, //mateB
                    {from: accounts[2]}),
                "Snail owner was unable to breed their snails"
            ...
            truffleAssert.eventEmitted(txBreedResult1, 'SnailsMated', (ev) => {
                ...
                return Number(ev.snailIdMateA) === A_SNAIL_ID &&
                    ...
                    Boolean(ev.mateBFertilised) === false
            }, "1. Event SnailsMated event has incorrect/unexpected parameter values!")

            // Breed for 2nd-time
            let txBreedResult2
            await truffleAssert.passes(
                txBreedResult2 = await snailToken.breedMateAFertilised(
                    A_SNAIL_ID, //mateA                
                    B_SNAIL_ID, //mateB
                    {from: accounts[2]}),
                "Snail owner was unable to breed their snails"
            ...
            let txBreedResult3
            await truffleAssert.passes(
                txBreedResult3 = await snailToken.breedMateAFertilised(
                    A_SNAIL_ID, //mateA                
                    B_SNAIL_ID, //mateB
                    {from: accounts[2]}),
                "Snail owner was unable to breed their snails"
        ...
        it("should get expected 1x conception from each breedMateAFertilised (test 3x)", async () => {
        ...
        it("should when there are new-born snails (from breedMateAFertilised), emit a 'SnailsBorn' event", async () => {

            let txBreedResult
            await truffleAssert.passes(
                txBreedResult = await snailToken.breedMateAFertilised(
                    A_SNAIL_ID, //mateA                
                    B_SNAIL_ID, //mateB
                    {from: accounts[2]}),
                "Snail owner was unable to breed their snails"
        ...
        it("should, after breedMateAFertilised indicates new-born snail(s), have minted the expected new snails", async () => {
    ...
    describe("Snail Breeding: Pausing  & Unpausing the SnailToken Contract", () => {
        ...
        it("should NOT breedMateAFertilised when SnailToken contract is in 'paused' state", async () => {
        ...
        it("should allow breedMateAFertilised after 'paused' SnailToken contract is 'unpaused'", async () => {
